...
const rateLimit = (ms = 5000) => new Promise(r => setTimeout(r, ms))
...
async function autoCheck() {
  ...
  try {
    ...
    if (acc.status === "active" && !acc.floodWaitUntil) return
...
setInterval(autoCheck, 15 * 60 * 1000) // 🔥 slow safe interval
