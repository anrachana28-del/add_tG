...
app.post('/add-member', async(req,res)=>{
  try{
    ...
    if(accountId){
      ...
      if(acc.floodWaitUntil && acc.floodWaitUntil>now){
        return res.json({status:"failed",reason:"Account in FloodWait"});
      }
    }else{
      if(!currentAccount){
        currentAccount = activeAccounts[accountIndex % activeAccounts.length];
      }
      acc = currentAccount;
    }
    ...
    try{
      await client.invoke(
        new Api.channels.InviteToChannel({channel:group, users:[user]})
      );

      /* RANDOM DELAY ONLY FOR SUCCESS */
      ...
      let joined=false;
      try{
        await client.invoke(new Api.channels.GetParticipant({channel:group, participant:user}));
        joined=true;
      ...
      if(errAdd.message.includes("USER_ALREADY_PARTICIPANT")){
        ...
        if(m){
          ...
          /* SWITCH ACCOUNT ON FLOOD */
          accountIndex++;
          currentAccount = activeAccounts[accountIndex % activeAccounts.length];

          status="failed";
          reason=`FloodWait until ${new Date(floodUntil).toLocaleTimeString('en-US',{hour12:true})}`;
        }
      }
      else{
        status="failed";
    ...
    res.json({status,accountUsed:acc.id,reason,delay});
...
app.post('/members', async(req,res)=>{
  try{
    ...
    if(!acc) return res.status(500).json({error:"All accounts FloodWait"});
